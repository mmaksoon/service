...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return  Scaffold(
        ...
        child: ListView(
          children:  [
             const UserAccountsDrawerHeader(
                ...
                accountName: Text('Воронков Максим'),
                accountEmail: Text("masy60955@gmail.com")),
            ...
            GestureDetector(
              ...
              child: const ListTile(
                title: Text("Выбрать услугу сервиса"),
      ...
      body: Container(
        child: StreamBuilder(
          stream: FirebaseFirestore.instance.collection('Заказы на запчасти').where('userId', isEqualTo: user!.uid).snapshots(),
                    ...
                    return Card(
                      child: FadeInLeftBig(
                        child: ListTile(
                          ...
                          subtitle: status == 'Ожидание' ? Text(snapshot.data!.docs[index]['status'],style: const TextStyle(color: Colors.white70),)
                              : Text(snapshot.data!.docs[index]['status'],style: const TextStyle(color: Colors.white70),),
                          ...
                          trailing: CircleAvatar(child: InkWell(
                              onTap: (){
                                Get.defaultDialog(
                                  ...
                                  onConfirm: () async {
                                    EasyLoading.show();
                                    await FirebaseFirestore.instance.collection('Запчасти').doc(docId).update({'status': 'Продано'},);
                                    Get.back();
                                    EasyLoading.dismiss();
                                  },
                                );
